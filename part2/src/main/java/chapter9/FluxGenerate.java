...
public class FluxGenerate {
    ...
    // code로 Flux를 만드는 가장 쉬운 방법인 generate -> (interface)SynchronousSink를 생성해줌
    ...
    public static void main(String[] args) {
        Flux<String> flux = Flux.generate( // generate 메소드를 Call하면 내부적으로 sink와 state를 만들어줌.
                () -> 0, // (1) state를 0으로 초기화한다
                (state, sink) -> { // this is implementation of apply method of BiFunction, sink is SynchronousSink
                    log.info("----- # generator: state = {}", state);
                    sink.next("3 x " + state + " = " + 3 * state); // (2) 데이터를 emit함. // onNext 호출
                    // onNext() emit할 파라미터 "3 x " + state + " = " + 3 * state 정의. subscribe의 data에 전달.

                    if (state == 10) sink.complete(); // (3) 중단 시점을 체크한다. // onComplete 호출
                    state += 1;
                  log.info("# ~ generator: state");
                  return state;
                  // return state + 1; // (4) 다음 실행에서 사용할 new state 값을 리턴한다.
                });

        // sink is a SynchronousSink and that its next() method can only be called at most once per callback invocation.
